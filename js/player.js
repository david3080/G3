...
(q.hasOwnProperty('type') &&
(q.type === 'sa' || q.type === 'button' || q.type === 'true-false')) ||
...
_cfg.settings.suspendable == true &&
...
until: new Date(new Date().getTime() + _cfg.settings.time_limit * 1000),
format: 'HMS',
...
until: new Date(
new Date().getTime() + _cfg.settings.time_limit * 1000 + 100
),
format: 'HMS',
tickInterval: 1,
compact: true
...
imgSize[srcData] = width + "x" + height;
initPhotoSwipeFromDOM('.my-gallery');
}

img.src = $(this).attr('src');
srcData = $(this).data("qgsrc");
});
if (_cfg.settings.math) {
/* cdn */ MathJax.Callback.Queue(
...
'<a href="' +
url +
'" data-size="600x400" class="imglink">' +
'<img src="' +
url +
'" data-original-src="' +
url +
...
if (
cq.match_key[
student_response.findIndex(function(j) {
...
$('#question .drag-choice-wordbank')
.eq(i)
.children('.disp_correct').length === 0
) {
$('#question .drag-choice-wordbank')
.eq(i)
.css('color', 'red');
...
'<span style="color:green">' + pulldown_answer + '</div>'
);
incorrect_cnt++;
} else {
...
if (
(cq.type === 'sa' || cq.type === 'button' || cq.type === 'true-false') &&
...
floor2ndDecimal(p * q.correctness) + '/' + floor2ndDecimal(p);
} else {
...
return floor2ndDecimal(score + 0.00001);
}
...
return floor2ndDecimal(
(score / _cfg.settings.question_count) * 100 + 0.00001
);
...
floor2ndDecimal(q.correctness * 100)
);
}
}
if (hasReport) {
